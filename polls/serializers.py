...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    choice_text = serializers.CharField(max_length=100, allow_null=True,
                                        required=False)
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError('Уже действие выполнено')
...
class ChoiceSerializer(serializers.Serializer):
    ...
    question = serializers.SlugRelatedField(queryset=Question.objects.all(),
                                            slug_field='id')
    choice_text = serializers.CharField(max_length=100)
...
class QuestionSerializer(serializers.Serializer):
    ...
    question_type = serializers.CharField(max_length=64)
...
class PollSerializer(serializers.ModelSerializer):
    ...
    poll_description = serializers.CharField(max_length=100)
    ...
    def update(self, instance, validated_data):
        if 'pub_date' in validated_data:
            raise serializers.ValidationError(
                {'pub_date': 'Изменить значение поля нельзя.'})
        for key, value in validated_data.items():
            setattr(instance, key, value)
        instance.save()
        return instance
